"""
Stage 5: feedforward residual MPC test.

Runs three trials on the perturbed plant (mass factor 1.2):
  1. Nominal MPC + nominal plant (baseline)
  2. Nominal MPC + perturbed plant (shows the gap to close)
  3. Feedforward Residual MPC + perturbed plant (the test)

Architecture: the nominal MPC solves as in Stage 3; then the residual
model predicts the next-step dynamics mismatch; we convert this into
a thrust correction and add it to the MPC's output before sending to
MuJoCo. The MPC's optimizer never sees the residual, so no IPOPT
failures.

Run:
    python3 04_feedforward_residual_test.py
"""
...
def run_one_trial(controller, mass_factor=None, label="trial",
                  log_residual=False):
    """
    Run one 15s circle trajectory with the given controller.

    If log_residual is True, also stores the residual predictions and
    thrust corrections at each step (only meaningful for the
    FeedforwardResidualController).
    """
    ...
    for step in range(n_steps):
        ...
        if log_residual and hasattr(controller, "last_residual"):
            log_residual_vz.append(float(controller.last_residual[5]))
            log_thrust_correction.append(float(controller.last_thrust_correction))
    ...
    if log_residual and log_thrust_correction:
        tc_arr = np.array(log_thrust_correction)
        rv_arr = np.array(log_residual_vz)
        print(f"    Residual vz: mean {rv_arr.mean():+.5f}, "
              f"std {rv_arr.std():.5f}")
        print(f"    Thrust correction: mean {tc_arr.mean():+.3f} N, "
              f"max |{np.abs(tc_arr).max():.3f}| N")
    ...
    return {
        ...
        "rms_cm": rms * 100,
        "max_cm": mx * 100,
        "solve_ms_mean": float(log_solve_ms.mean()),
        "solve_ms_max": float(log_solve_ms.max()),
        "thrust_correction": np.array(log_thrust_correction)
                              if log_thrust_correction else None,
        "residual_vz": np.array(log_residual_vz)
                       if log_residual_vz else None,
    }


def main():
    print("=" * 60)
    print("Stage 5 — feedforward residual MPC test")
    print("=" * 60)
    ...
    feedforward_residual = FeedforwardResidualController(
    ...
    trial_nom_perturbed = run_one_trial(
        MPCController(), mass_factor=PERTURB_MASS,
        label=f"Nominal MPC + plant mass={PERTURB_MASS}"
    )
    trial_ff_perturbed = run_one_trial(
        feedforward_residual, mass_factor=PERTURB_MASS,
        label=f"Feedforward Residual + plant mass={PERTURB_MASS}",
        log_residual=True,
    )
    ...
    print("SUMMARY")
    print("=" * 60)
    print(f"  {'Trial':<50} {'RMS (cm)':>10} {'Max (cm)':>10}")
    print(f"  {'-'*50} {'-'*10} {'-'*10}")
    for trial in [trial_nom_nom, trial_nom_perturbed, trial_ff_perturbed]:
        print(f"  {trial['label']:<50} "
              f"{trial['rms_cm']:>10.2f} {trial['max_cm']:>10.2f}")

    gap = trial_nom_perturbed['rms_cm'] - trial_nom_nom['rms_cm']
    recovered = trial_nom_perturbed['rms_cm'] - trial_ff_perturbed['rms_cm']
    if gap > 0:
        pct = recovered / gap * 100
        print(f"\n  Performance gap (perturbed - nominal): {gap:.2f} cm")
        print(f"  Recovered by feedforward residual:      {recovered:.2f} cm "
              f"({pct:.1f}%)")
    ...
    ax = axes[2]
    if trial_ff_perturbed["thrust_correction"] is not None:
        ax.plot(trial_ff_perturbed["t"],
                trial_ff_perturbed["thrust_correction"],
                color="C4", label="thrust correction")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Δthrust (N)")
        ax.set_title("Feedforward thrust correction")
        ax.grid(alpha=0.3)
        ax.legend(fontsize=8)

    out_path = os.path.join(HERE, "feedforward_residual_test.png")
    plt.tight_layout()

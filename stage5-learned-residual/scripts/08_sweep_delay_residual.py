"""
Stage 5 evaluation — time delay perturbation sweep with feedforward residual.

For each delay in {0, 5, 10, 14} ms:
  1. Nominal MPC + nominal plant (baseline, run once)
  2. Nominal MPC + time delay perturbation
  3. Feedforward Residual + time delay perturbation

Delay values stay below the 15 ms cliff Stage 4 found.

Run:
    python3 08_sweep_delay_residual.py
"""
...
# Delay values in seconds. Stay below the 15 ms cliff.
DELAYS = [0.0, 0.005, 0.010, 0.014]

OUT_CSV = os.path.join(HERE, "sweep_delay_residual.csv")
OUT_PNG = os.path.join(HERE, "sweep_delay_residual.png")


def main():
    print("=" * 60)
    print("Stage 5 — time delay sweep with feedforward residual")
    print("=" * 60)
    ...
    for delay in DELAYS:
        ...
        print(f"\n[nominal MPC, delay={delay_ms} ms]")
        ctrl = MPCController()
        spec = {"type": "time_delay", "value": delay}
        res = run_trial(ctrl, spec)
        print(f"  RMS = {res['rms_cm']:.2f} cm  crashed={res['crashed']}")
        rows.append({
            "config": "nominal_perturbed", "delay_s": delay,
            "controller": "nominal", "perturbed": True, **res,
        })

        print(f"\n[ff residual, delay={delay_ms} ms]")
        ...
        rows.append({
            "config": "ff_residual_perturbed", "delay_s": delay,
    ...
    ax = axes[0]
    ax.plot(delays_ms, nom_rms, "o-", color="C3", label="Nominal MPC")
    ax.plot(delays_ms, ff_rms, "o-", color="C0",
    ...
    ax.set_title("Tracking error vs time delay")
    ax.grid(alpha=0.3)
    ...
    ax.bar(delays_ms, pct, width=1.5, color=bar_colors)
    ...
    for i, d in enumerate(DELAYS):
        if not np.isnan(pct[i]):
            ax.text(delays_ms[i], pct[i] + 3, f"{pct[i]:.0f}%",
    ...
    print("\nSUMMARY")
    print(f"  {'delay(ms)':>9} {'nominal (cm)':>14} {'ff resid (cm)':>14} "
          f"{'closure':>10}")
    for i, d in enumerate(DELAYS):
        c = "n/a" if np.isnan(pct[i]) else f"{pct[i]:.0f}%"
        print(f"  {delays_ms[i]:>9.0f} {nom_rms[i]:>14.2f} "
